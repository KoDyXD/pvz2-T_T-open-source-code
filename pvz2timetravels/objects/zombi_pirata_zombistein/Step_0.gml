...
if hp <= 0
{
	if anim_nada = false
	{
		...
		with(i)
		{
			...
			if other.zombidito = false	//skin sin imp
			{
				skeleton_skin_set("normal/muerto sin imp");
			}
			else	//skin con imp
			{
				skeleton_skin_set("normal/muerto con imp");
			}
...
if estatico = false
{
	...
	{
		detener = true;
		if image_index >= 44 and zombidito = true
		{
			var i = instance_create_layer(x+165,y-325,"npcs",proj_zombi_garg_imp);
			with(i)
			{
				sprite_index = spr_zombi_pirata_imp_anims;
				skeleton_skin_set("normal/entero");
				skeleton_animation_set("basico/lanzado");
				y2 = other.y;
				x1 = other.x+165;
				y1 = other.y-325;
				sombra_y = other.y;
				zombi = zombi_pirata_zombidito;
			}
