...
if recompensa = true and not instance_exists(zombi_tipo_general)
{
	...
	{
		if global.nivel_egipto[15] = false	//si es la primera vez que se completa el nivel
		{
			//guardar el progreso del nivel
			global.cuentaniveles += 1;
			global.nivel_egipto[15] = true;
			ini_open(global.nombre);
			ini_write_real("Nivel","completados",global.cuentaniveles);
			ini_write_real("Nivel","egipto_1_15", global.nivel_egipto[15]);
			ini_close();
			instance_create_layer(global.xz, global.yz, "hud", recom_sem_birasol);
		}
