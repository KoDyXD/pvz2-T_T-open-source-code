...
if !audio_group_is_loaded(audiogroup2)
{
...
{
	if !audio_is_playing(music_cuevas_recompensa) and !instance_exists(obj_transicion) {audio_sound_gain(duracion, 0, 800);}
	if !audio_is_playing(music_cuevas_recompensa) and !instance_exists(obj_transicion) {duracion = audio_play_sound(music_cuevas_recompensa, 1000, 0);}
	if instance_exists(obj_transicion) and !audio_is_playing(music_cuevas_recompensa) {if obj_transicion.intensidad >= 1 {alarm_set(1,1); ganar = false;}}
	intro = false;
	oleada_1 = false;
	oleada_2 = false;
	oleada_3 = false;
...
if perder = true and global.perder = true
{
	...
	perder = false;
	audio_sound_gain(duracion,0, 800);
	audio_play_sound(music_cuevas_perder,1000,0);
