...
if instance_exists(padre) and alarm[1] = -1
{
	...
	if accion == "aplicar"
	{
		if padre.grupo_datos[2] != "" and padre.grupo_datos[2] != "-" and padre.grupo_datos[3] != "" and padre.grupo_datos[3] != "-" and padre.grupo_datos[5] != "" and padre.grupo_datos[5] != "-" and padre.grupo_datos[6] != "" and padre.grupo_datos[6] != "-"
		{
			if real(padre.grupo_datos[6]) >= real(padre.grupo_datos[5])
			{
				if modo == "modificar"
				{
	...
	{	
		if collision_rectangle(_x+61,_y-340,_x+221,_y-260,obj_click,0,1) and mouse_check_button_pressed(mb_left) and cual != "x"
		{
			audio_play_sound(sound_menu_boton_perfil,5,0);
			cual = "x";
			array_valor = 2;
		}
		if collision_rectangle(_x+61,_y-190,_x+221,_y-110,obj_click,0,1) and mouse_check_button_pressed(mb_left) and cual != "y"
		{
			audio_play_sound(sound_menu_boton_perfil,5,0);
			cual = "y";
			array_valor = 3;
		}
		if collision_rectangle(_x+61,_y-40,_x+221,_y+40,obj_click,0,1) and mouse_check_button_pressed(mb_left) and cual != "c_min"
		{
			audio_play_sound(sound_menu_boton_perfil,5,0);
			cual = "c_min";
			array_valor = 5;
		}
		if collision_rectangle(_x+61,_y+110,_x+221,_y+190,obj_click,0,1) and mouse_check_button_pressed(mb_left) and cual != "c_max"
		{
			audio_play_sound(sound_menu_boton_perfil,5,0);
			cual = "c_max";
			array_valor = 6;
		}
		if collision_rectangle(_x+61,_y+260,_x+221,_y+340,obj_click,0,1) and mouse_check_button_pressed(mb_left) and cual != "zombi"
		{
			audio_play_sound(sound_menu_boton_perfil,5,0);
			cual = "zombi";
			array_valor = 4;
		...
		if cual == "zombi"
		{
			if !instance_exists(editor_menu_oleadas_selector_zombi)
			{
				var xx = -1;
				var yy = 0;
				...
				for (var i = 1; i < array_length(zombi); ++i) {
					if xx >= 2 {xx = -1; yy += 1;}
					xx += 1;
				    var ii = instance_create_layer(_x+200*xx,_y+126*yy,"hud",editor_menu_oleadas_selector_zombi);
					ii.cual = zombi[i];
					ii.padre = id;
				}
			}
		}
		else if instance_exists(editor_menu_oleadas_selector_zombi)
		{
			instance_destroy(editor_menu_oleadas_selector_zombi);
		}
	}
	...
	//ajustar los valores de la generacion de zombis
	if padre.grupo_datos[2] != "" and padre.grupo_datos[2] != "-"
	{
		...
		if real(padre.grupo_datos[2]) > 4 {padre.grupo_datos[2] = "4";}
	}
	if padre.grupo_datos[3] != "" and padre.grupo_datos[3] != "-"
	{
		//ajustar los valores de Y
		if real(padre.grupo_datos[3]) < -10 {padre.grupo_datos[3] = "-10";}
		if real(padre.grupo_datos[3]) > 5 {padre.grupo_datos[3] = "5";}
	}
	if padre.grupo_datos[5] != "" and padre.grupo_datos[5] != "-"
	{
		//ajustar para que la cantidad minima a generar no sea negativa
		if real(padre.grupo_datos[5]) < 0 {padre.grupo_datos[5] = "0";}
	}
	if padre.grupo_datos[6] != "" and padre.grupo_datos[6] != "-"
	{
		//ajustar para que la cantidad maxima a generar no sea negativa
		if real(padre.grupo_datos[6]) < 0 {padre.grupo_datos[6] = "0";}
	}
}