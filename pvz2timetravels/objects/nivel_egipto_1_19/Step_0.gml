...
if recompensa = true and not instance_exists(zombi_tipo_general)
{
	...
	{
		if global.nivel_egipto[19] = false	//si es la primera vez que se completa el nivel
		{
			//guardar el progreso del nivel
			global.cuentaniveles += 1;
			global.nivel_egipto[19] = true;
			ini_open(global.nombre);
			ini_write_real("Nivel","completados",global.cuentaniveles);
			ini_write_real("Nivel","egipto_1_19", global.nivel_egipto[19]);
			ini_close();
...
if instance_number(tumba_egipto) <= 4 and alarm[3] = -1
{
	instance_create_tumbas(3,1,9,5,2,4,tumba_egipto);
}
#endregion
