...
{
	global.nivel_cual = nivel_egipto_1_5;	//siguiente nivel
	if global.trucos == false	//los trucos estan desactivados
	{
		if global.nivel_egipto[4] = false	//si es la primera vez que se completa el nivel
		{
			//guardar el progreso del nivel
			global.cuentaniveles += 1;
			global.nivel_egipto[4] = true;
			ini_open(global.nombre);
			ini_write_real("Nivel","completados",global.cuentaniveles);
			ini_write_real("Nivel","egipto_1_4", global.nivel_egipto[4]);
			ini_close();
			instance_create_layer(global.xz, global.yz, "hud", recom_sem_hielaguisantes);
		}
